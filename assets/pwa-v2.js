...
(function(){
  ...
  const KDA=window.KDAData;let corePromise=null,crossPromise=null,seriesPromise=null,dataPromise=null;
  ...
  function currentView(){return window.KDARouter?.current?.()?.view||window.KDARouter?.parse?.()?.view||'home';}
  ...
  function onCompare(){if(currentView()!=='compare')return;if(crossPromise)return;const base=window.KDAOptional?.loadCompare?.()||Promise.resolve();crossPromise=Promise.resolve(base).then(()=>Promise.all([style('assets/compare-cross-level.css','kda-compare-cross-level-css'),script('assets/compare-cross-level.js','kda-compare-cross-level-js')])).then(()=>window.KDACompareCross?.boot?.()).catch(error=>{console.warn('Cross-level Compare load:',error?.message||error);return null;});}
  function onSeries(){if(currentView()!=='series')return;if(seriesPromise){window.KDASeriesV2?.render?.();return;}const base=window.KDAOptional?.loadSeriesBrowser?.()||Promise.resolve();seriesPromise=Promise.resolve(base).then(()=>Promise.all([style('assets/series-v2-workbench.css','kda-series-v2-workbench-css'),script('assets/series-v2-workbench.js','kda-series-v2-workbench-js')])).then(()=>window.KDASeriesV2?.render?.()).catch(error=>{console.warn('Series v2 workbench load:',error?.message||error);return null;});}
  function onData(){if(currentView()!=='data')return;if(dataPromise){window.KDADataLineageV2?.render?.();return;}dataPromise=Promise.all([style('assets/data-lineage-v2.css','kda-data-lineage-v2-css'),script('assets/data-lineage-v2.js','kda-data-lineage-v2-js')]).then(()=>window.KDADataLineageV2?.render?.()).catch(error=>{console.warn('Data lineage v2 load:',error?.message||error);return null;});}
  function loadRouteExtras(){patchSearchLoader();onCompare();onSeries();onData();}
  loadCore().then(loadRouteExtras);
  window.addEventListener('kda:route',loadRouteExtras);
})();
