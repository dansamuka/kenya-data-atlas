...
for(const row of rows){
  ...
  assert(Number(geo.constituency_code)===Number(row.constituency_code),`${row.geo_code}: constituency code mismatch`);
...
console.log(`P23_FORM34B_SOURCE_MANIFEST_OK canonical=${rows.length} aliases=${seenAliases.size} excluded=${excluded.length} form_refs=${formIds.length} values_promoted=0`);
