...
if (mode === 'catalogue') {
  ...
  if (!releases.some(r => r.release_code === rcode)) {
    releases.push({
      release_id: uuid(`release:${rcode}`),
      release_code: rcode,
      dataset_id: ds.dataset_id,
      title: 'IEBC 2022 Presidential Form 34B — source-verified constituency turnout',
      reference_period_start: '2022-08-09',
      reference_period_end: '2022-08-09',
      published_at: '',
      discovered_at: INGESTED_AT,
      ingested_at: INGESTED_AT,
      release_url: PORTAL_URL,
  ...
  for (const row of verified) {
    ...
    if (!s) {
      s = {
        ...
        comparability_group: 'IEBC-PRESIDENTIAL-TURNOUT-2022-FORM34B',
        dataset_id: ds.dataset_id,
        agency_id: agency,
        methodology_url: 'data/p23/form34b-extraction-contract.json',
        start_period: '2022-08-09',
        end_period: '2022-08-09',
    ...
    if (!obsIds.has(oid)) {
      observations.push({
        ...
        period_end: '2022-08-09',
        period_type: 'point_in_time',
        period_label: '2022 presidential election',
        value: row.turnout,
        geographic_method: 'direct',
        ...
        source_table: 'Form 34B — final TOTAL row',
        source_sheet: '',
        source_page: String(sample.page_number),
        source_row_label: 'TOTAL',
        source_url: sample.source_url,
        published_at: '',
        ingested_at: INGESTED_AT,
        vintage_id: uuid(`vintage:${scode}:2022-08-09:1`),
