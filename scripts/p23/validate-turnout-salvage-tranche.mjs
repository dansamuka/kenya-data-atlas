...
const allowedOutcomeReasons = new Set(['official_pdf_missing_results_pages', 'denominator_mismatch']);
...
for (const [index, outcome] of Array.isArray(outcomeRows) ? outcomeRows.entries() : []) {
  ...
  if (outcome.reason === 'official_pdf_missing_results_pages') {
    ...
    if (outcome.source_review_file !== undefined) fail(`${where}: missing-results closure must not claim a numeric source-review file`);
  }

  if (outcome.reason === 'denominator_mismatch') {
    if (review.constituency_identity_visually_confirmed !== true) fail(`${where}: denominator mismatch requires visually confirmed constituency identity`);
    if (review.result_table_present !== true) fail(`${where}: denominator mismatch requires a visible results table`);
    if (review.total_row_present !== true) fail(`${where}: denominator mismatch requires a visible TOTAL row`);
    if (typeof outcome.source_review_file !== 'string' || !/^data\/p23\/form34b-.+-fresh-source-review\.json$/.test(outcome.source_review_file)) {
      fail(`${where}: denominator mismatch requires a governed fresh source-review file`);
    } else {
      const sourceReviewPath = path.join(root, outcome.source_review_file);
      if (!fs.existsSync(sourceReviewPath)) {
        fail(`${where}: governed fresh source-review file is missing`);
      } else {
        const sourceReview = JSON.parse(fs.readFileSync(sourceReviewPath, 'utf8'));
        if (sourceReview.geo_code !== outcome.geo_code || sourceReview.constituency_name !== outcome.name) fail(`${where}: fresh source-review identity mismatch`);
        if (sourceReview.verification_state !== 'denominator_mismatch') fail(`${where}: linked source review must remain denominator_mismatch`);
        if (sourceReview.promotion_eligible !== false) fail(`${where}: linked source review must remain non-promotable`);
        if (sourceReview.promotion_authorized_by_this_file !== false) fail(`${where}: linked source review must not authorize promotion`);
        if (sourceReview.canonical_turnout_value_written !== false) fail(`${where}: linked source review must not write canonical turnout`);
        if (sourceReview.governed_denominator_reconciliation?.registered_voters_reconciles !== false) fail(`${where}: linked source review no longer has a denominator mismatch`);
        if (sourceReview.governed_denominator_reconciliation?.special_facility_exactly_explains_delta !== true) fail(`${where}: linked denominator mismatch lacks the governed special-facility reconciliation`);

        const fresh = sourceReview.fresh_source || {};
        if (fresh.render_dpi !== 250) fail(`${where}: linked source review must remain exactly 250 DPI`);
        if (fresh.workflow_run_id !== provenance.workflow_run_id) fail(`${where}: linked source-review workflow run id mismatch`);
        if (fresh.workflow_head_sha !== provenance.workflow_head_sha) fail(`${where}: linked source-review workflow head SHA mismatch`);
        if (fresh.source_pdf_sha256 !== provenance.source_pdf_sha256) fail(`${where}: linked source PDF hash mismatch`);
        if (fresh.review_context_image_sha256 !== context.review_context_image_sha256) fail(`${where}: linked review-context hash mismatch`);
        if (fresh.page_number !== context.reviewed_pdf_page_number) fail(`${where}: linked reviewed page mismatch`);
      }
    }
  }

  if (outcome.source_verification_granted !== false) fail(`${where}: fresh non-promotable outcome must not grant row-level source verification`);
