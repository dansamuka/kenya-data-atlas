...
const tranchePath = path.join(root, `data/p23/turnout-salvage-tranche-${trancheMatch[1]}.json`);
...
for (let i = 0; i < (rows || []).length; i += 1) {
  ...
  if (row?.geo_code !== exp?.geo_code || row?.name !== exp?.name) fail(`row ${i + 1} does not match canonical ${trancheId} order`);
...
if (!process.exitCode) {
  console.log(`P23 salvage fresh-download validation passed: tranche=${trancheId} rows=${rows.length} downloaded_pdf=${downloaded}; locator-only source-index formula; no values; no promotion; 250-DPI review remains future-gated.`);
}
