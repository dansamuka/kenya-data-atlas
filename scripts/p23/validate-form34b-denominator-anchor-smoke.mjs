...
if (!Number.isInteger(doc.batch_offset) || doc.batch_offset < 0 || doc.batch_offset >= 290) fail('Invalid governed batch_offset');
...
if (doc.batch_offset + doc.rows_processed > 290) fail('Denominator locator batch exceeds governed 290-row manifest');
...
if (doc.batch_offset === 0 && !doc.rows.some((row) => row.geo_code === 'KEN-C001-CON001')) fail('Governed Changamwe row missing from first diagnostic batch');
...
console.log(`P23_FORM34B_DENOMINATOR_ANCHOR_VALID offset=${doc.batch_offset} rows=${doc.rows_processed} unique=${unique} ambiguous=${ambiguous} source_verified_values=0 promotion_authorized=false`);
