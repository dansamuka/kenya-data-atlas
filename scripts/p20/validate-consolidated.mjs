...
const indicators=['IND-TEENAGE-PREGNANCY','IND-HOME-BIRTH-RATE','IND-DISABILITY-PREVALENCE','IND-CONTRACEPTIVE-USE','IND-FGM-CHILD-MARRIAGE'];
try{
  assert(summary.total_slots===20115,`governed slot denominator changed: ${summary.total_slots}`);
  assert(summary.resolved_slots===3103,`expected 3,103 resolved slots, got ${summary.resolved_slots}`);
  assert(summary.unresolved_slots===17012,`expected 17,012 unresolved slots, got ${summary.unresolved_slots}`);
  assert(summary.by_completion_phase?.P20===282,`expected 282 P20 slots remaining, got ${summary.by_completion_phase?.P20}`);
