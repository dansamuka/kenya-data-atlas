...
const indicators=['IND-TEENAGE-PREGNANCY','IND-HOME-BIRTH-RATE','IND-DISABILITY-PREVALENCE','IND-CONTRACEPTIVE-USE','IND-FGM-CHILD-MARRIAGE','IND-LITERACY-RATE','IND-HOUSING-MATERIAL','IND-HIV-PREVALENCE'];
try{
  assert(summary.total_slots===20115,`governed slot denominator changed: ${summary.total_slots}`);
  assert(summary.resolved_slots===3244,`expected 3,244 resolved slots, got ${summary.resolved_slots}`);
  assert(summary.unresolved_slots===16871,`expected 16,871 unresolved slots, got ${summary.unresolved_slots}`);
  assert(summary.by_completion_phase?.P20===141,`expected 141 P20 slots remaining, got ${summary.by_completion_phase?.P20}`);
  ...
  assert(Object.keys(grouped).length===3,`expected three remaining P20 families, got ${JSON.stringify(grouped)}`);
  ...
  for(const code of ['IND-COUNTY-PENDING-BILLS','IND-HEALTH-FACILITY-DENSITY','IND-SUBSTANCE-ABUSE-PREVALENCE']) assert(grouped[code]===47,`${code}: must remain a 47-county unresolved family until its own semantic/source gate is satisfied`);
