...
function validateSample(){
  ...
  for(const row of sample.rows){
    ...
    assert(Number(br.overall_absorption_pct)===Number(row.absorption),`${row.geo_code} overall absorption does not match source snapshot`);
    assert(Number(vr.value)===Number(row.voters),`${row.geo_code} voters do not match source snapshot`);
  }
...
function validateUiLabels(){
  ...
  console.log('COUNTYIQ_DEMO_LABELS_OK');
}
