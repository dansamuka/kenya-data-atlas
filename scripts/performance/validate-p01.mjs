...
function validateSharedLoader(){
  for(const token of ['jsonCache=new Map()','registry(name','registries(names','geometry(level','ensureD3','whenVisible','window.KDAData']) assert(loader.includes(token),`shared loader missing ${token}`);
...
function validateLazyConsumers(){
  ...
  assert(app.includes("KDA.registries(['geographies','indicators'])"),'search should load metadata only on demand');
