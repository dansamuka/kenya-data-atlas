...
const p23ConstituencyTurnoutCodes=new Set(['IND-TURNOUT-HISTORY']);
...
const p23ConstituencyTurnoutUnavailable=officialUnavailable.filter(s=>s.level==='constituency'&&p23ConstituencyTurnoutCodes.has(s.indicator_code));
...
assert(p23ConstituencyTurnoutUnavailable.length===94,`P23 constituency turnout closure must contribute exactly 94 governed official-unavailable states, got ${p23ConstituencyTurnoutUnavailable.length}`);
...
assert(officialUnavailable.length===5735,`official-unavailable evidence inventory must reconcile 48 legacy + 66 P22 + 580 P23 census + 580 P23 evidence gaps + 94 P23 constituency turnout + 1450 P24 ward census + 1450 P24 ward evidence gaps + 1450 P24 ward turnout + 17 P24 ward MCA identity = 5735, got ${officialUnavailable.length}`);
assert(p23ConstituencyTurnoutUnavailable.every(s=>s.as_of==='2026-09-11'&&String(s.evidence_constraint||'').length>0),'P23 constituency turnout unavailable states must retain snapshot date and evidence constraint');
...
assert(p24McaIdentityUnavailable.every(s=>s.as_of==='2026-09-09'&&String(s.evidence_constraint||'').length>0),'P24 ward MCA identity states must retain snapshot date and evidence constraint');
assert(p24McaIdentityUnavailable.every(s=>String(s.refresh_trigger||'').length>0),'P24 ward MCA identity states must retain refresh triggers');
