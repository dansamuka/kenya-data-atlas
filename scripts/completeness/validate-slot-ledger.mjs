...
const legacyUnavailable=officialUnavailable.filter(s=>!p22Codes.has(s.indicator_code)&&!(s.level==='constituency'&&(p23CensusCodes.has(s.indicator_code)||p23EvidenceGapCodes.has(s.indicator_code)))&&!(s.level==='ward'&&(p24CensusCodes.has(s.indicator_code)||p24WardEvidenceGapCodes.has(s.indicator_code))));
...
assert(officialUnavailable.length===4174,`official-unavailable evidence inventory must reconcile 48 legacy + 66 P22 + 580 P23 census + 580 P23 evidence gaps + 1450 P24 ward census + 1450 P24 ward evidence gaps = 4174, got ${officialUnavailable.length}`);
