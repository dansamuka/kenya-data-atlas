...
const legacyUnavailable=officialUnavailable.filter(s=>!p22Codes.has(s.indicator_code)&&!(s.level==='constituency'&&(p23CensusCodes.has(s.indicator_code)||p23EvidenceGapCodes.has(s.indicator_code)))&&!(s.level==='ward'&&p24WardTurnoutCodes.has(s.indicator_code)));
...
assert(officialUnavailable.length===2724,`official-unavailable evidence inventory must reconcile 48 legacy + 66 P22 + 580 P23 census + 580 P23 evidence gaps + 1450 P24 ward turnout = 2724, got ${officialUnavailable.length}`);
...
console.log(`P18_P22_P23_P24_UNAVAILABLE_RECONCILIATION_OK legacy=${legacyUnavailable.length} p22=${p22Unavailable.length} p23_census=${p23CensusUnavailable.length} p23_evidence_gaps=${p23EvidenceGapUnavailable.length} p24_ward_turnout=${p24WardTurnoutUnavailable.length} total=${officialUnavailable.length}`);
