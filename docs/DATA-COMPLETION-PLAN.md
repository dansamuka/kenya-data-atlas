...
**P23 constituency-turnout closure — governed `official_unavailable` for genuinely blocked Form 34B rows:** as of **11 September 2026**, **48** of the 290 constituency `IND-TURNOUT-HISTORY` slots are closed as governed `official_unavailable`. These 48 were independently investigated across six parallel P23 Form 34B source-verification tranches (merged pull requests #135-#140), each of which found and documented a genuine, non-fabrication reason the row could not be promoted -- a registered-voters denominator conflict between the printed Form 34B TOTAL row and the governed canonical P23 constituency denominator (29 constituencies, six of them traced to a specific named special-facility polling station -- a prison or similar -- whose own registered-voter count exactly explains the gap), an internal candidate-vote arithmetic mismatch on the printed form itself (10 constituencies), a structural scan defect such as a missing Rejected Ballots column or two disagreeing official tallies on the same document (4 constituencies), or a reproducible IEBC Result Forms Portal access failure serving the wrong or an incomplete PDF (3 constituencies, plus 2 constituencies affected by both a denominator and an arithmetic mismatch). That documentation previously existed only in the merged pull-request descriptions on GitHub; it is now formalised as persisted, per-constituency evidence in `data/p23/constituency-turnout-unavailable-contract.json` and applied via `scripts/p23/close-constituency-turnout-unavailable.mjs`. No turnout value is computed, estimated, or promoted for any of the 48; each closure carries a specific, per-constituency refresh trigger. The remaining unresolved P23 turnout queue -- **109** constituencies with no extraction attempt documented anywhere in this repository's history -- is listed in `data/p23/never-attempted-constituencies.json` for a follow-up extraction effort.
