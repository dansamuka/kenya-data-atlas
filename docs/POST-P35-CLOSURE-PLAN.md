...
- **Finalisation pass** (~10 hours later, same day): re-checked both tranche families again — no change. Also investigated the next most-tractable candidate outside the two blocked domains — **R077** (registered voters, Mandera East/Lafey ward crosswalk, only 10 cells but not dependent on any external live system) — and confirmed no authoritative IEBC ward-boundary GIS source exists publicly at all for this constituency pair (IEBC publishes only tabular voter-registration PDFs, never spatial data), so this remains genuinely unresolved rather than merely unresearched. With every readily-available avenue confirmed blocked twice over, P41 is finalised in this state for now: honestly open, zero yield, with a complete and reproducible due-diligence record rather than either a fabricated promotion or an abandoned phase.
...
- **Checked the 20 feasibility A/B rows before attempting execution**: all 20 (beyond `already_numeric`) are the exact same `publication_pending` (8, National School Census) and `access_technical` (12, KMHFR+KilimoSTAT) reason groups already exhaustively verified blocked, twice, in P41 — nothing new to execute there right now.
- Of the 60 feasibility-C rows, 50 are `small_area_estimation` (require microdata; gated on the still-running independent KeNADA follow-up investigation) and 10 are `constrained_downscaling` for GCP/manufacturing indicators, for which no genuinely defensible disaggregation covariate is currently in hand — this project's own prior research already explicitly rejected population/land-area allocation as fabrication for these exact indicators, and forcing a "modelled" value without a real covariate would just be that same rejected technique with extra steps.
- **Built the shared validation gate instead of fabricating a value or waiting idly**: [`scripts/p42/validation-gate.mjs`](../scripts/p42/validation-gate.mjs) (pure, 13 unit tests, including deliberate-failure paths) implements the contract's own `validation_framework` rules — back-aggregation reconciliation against an authoritative parent total, hold-out mean-absolute-percentage-error checks, and a single `decidePublication()` choke point that refuses to publish unless every declared threshold genuinely passes. No default threshold exists anywhere in the module; every evaluation function throws if asked to judge without one explicitly declared by the calling indicator family. This produces zero modelled values — it is infrastructure for when real data becomes available, not a shortcut around needing it.
